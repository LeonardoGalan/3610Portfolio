...
let startGameText =
  "Hello my name is Leonardo Galan, \n\nThis game is about collecting stars \nand avoiding the bomb that jumps \naround the screen. You have 3 lives \nand every time you get hit by a bomb \nyou lose one life. When all lives are lost, the game is over!";
function preload() {
...
function create() {
  ...
  //creates the texts for the score
  ...
  newGame = this.add.text(1000, 130, "Press M for New game", {
    fontSize: "15px",
  });
  ...
  startMessage = this.add.text(16, 16, startGameText, {
    fontSize: "32px",
    fill: "#ffffff",
  });
  pressClick = this.add.text(350, 500, "PRESS CLICK TO PLAY", {
    fontSize: "40px",
    fill: "#ffffff",
  });
...
function update() {
  ...
  if (mKey.isDown && gameStarted && !newGameOpen && !inHelp) {
    ...
    startMessage = this.add.text(510, 200, "Are you sure", {
      fontSize: "25px",
    });
...
function hitBomb(player, bomb) {
  if (lives === 1) {
    ...
    pressClick = this.add.text(490, 450, "Press N for new game", {
      fontSize: "20px",
    });
  }
